import type { BoardMode, BoardPlayer, Club, FormationCoordinate, League, NationalTeam, Player, PlayerPoolType } from '../types';
import { FormationSelector } from './FormationSelector';
import { PlayerSelector } from './PlayerSelector';
import { formations } from '../data/mockData';
import { getPoolLabel, getPlayersForPool } from '../utils/playerPools';

interface Props { mode: BoardMode; playerPoolType: PlayerPoolType; selectedPoolId: string; clubs: Club[]; leagues: League[]; nationalTeams: NationalTeam[]; players: Player[]; boardPlayers: BoardPlayer[]; formationCoordinates: FormationCoordinate[]; activeRoleIndex: number | null; formationId: string; selectedIds: string[]; onModeChange: (mode: BoardMode) => void; onPoolTypeChange: (type: PlayerPoolType) => void; onPoolChange: (poolId: string) => void; onFormationChange: (id: string) => void; onSpotSelect: (index: number) => void; onLoadExampleTeam: () => void; onTogglePlayer: (id: string) => void; }
