...
import type { BoardMode, BoardPlayer, CameraAngle, Club, Player } from '../types';
import { clampPitchCoordinate } from '../utils/formation';
import { PlayerMarker } from './PlayerMarker';

interface Props { boardPlayers: BoardPlayer[]; players: Player[]; clubs: Club[]; mode: BoardMode; cameraAngle: CameraAngle; onMovePlayer: (playerId: string, x: number, y: number) => void; }

export function Pitch({ boardPlayers, players, clubs, mode, cameraAngle, onMovePlayer }: Props) {
  ...
  return <div className={`pitch-stage camera-${cameraAngle}`}><div ref={pitchRef} className="pitch" onPointerMove={(event) => draggingId && movePlayerFromPointer(draggingId, event.clientX, event.clientY)} onPointerUp={() => setDraggingId(null)} onPointerCancel={() => setDraggingId(null)}>
