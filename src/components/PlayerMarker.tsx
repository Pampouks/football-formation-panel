...
interface Props { player: Player; club?: Club; mode: BoardMode; x: number; y: number; role: string; isDragging: boolean; markerScale: number; kitColor: string; kitImageUrl?: string; showLabel: boolean; onPointerDown: (playerId: string, event: PointerEvent<HTMLDivElement>) => void; }
const initials = (name: string) => name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase();

export function PlayerMarker({ player, club, mode, x, y, role, isDragging, markerScale, kitColor, kitImageUrl, showLabel, onPointerDown }: Props) {
